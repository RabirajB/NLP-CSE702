...
def main(filenames, label, csvname = None, neutral_lists = [], group_lists = ['male_pairs', 'female_pairs'], do_individual_group_words = False, do_individual_neutral_words = False, do_cross_individual = False):
    ...
    for grouplist in group_lists:
        with open(datafolder+grouplist + '.txt', 'r') as f2:
    ...
    for neuten, neut in enumerate(neutral_lists):
        with open(datafolder+neut + '.txt', 'r') as f:
    ...
    with open(result_folder + 'run_results/'+csvname, 'a') as cf:
...
folder = 'E:/Joseph-Seminar/EmbeddingDynamicStereotypes-master/vectors/normalized_clean/'
datafolder = "E:/Joseph-Seminar\EmbeddingDynamicStereotypes-master/data/"
result_folder = "E:/Joseph-Seminar/EmbeddingDynamicStereotypes-master/"
...
filename_map = {'sgns' : filenames_sgns, 'svd': filenames_svd, 'google':filenames_google, 'wikipedia':filenames_wikipedia}

if __name__ == "__main__":
    param_filename = 'run_params.csv'

    with open(param_filename,'r', encoding='utf8') as f:
